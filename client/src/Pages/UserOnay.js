...
import { Post, Get } from "../Helper/Service";
...
export default class Login extends Component {
    ...
    async componentDidMount() {
        ...
        var path = window.location.pathname.split("/");
        let UserOnayResponse = await Get("UserOnay", path[2]);
        if (UserOnayResponse.success) {
            toast.success("Kullanıcı Onaylama Başarılı Lütfen Giriş Yapın");
        }
    }
    render() {
        return (
